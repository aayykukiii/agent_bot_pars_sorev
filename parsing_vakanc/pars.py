                ...
                if response.status == 200:
                    ...
                    for i, vacancy in enumerate(vacancies, 1):
                        try:
                            ...
                            date_elem = vacancy.find('time', class_='vacancy-card__date')
                            if not date_elem:
                                date_elem = vacancy.find('div', class_='vacancy-card__date')
                            if not date_elem:
                                date_elem = vacancy.find('time')
                            if not date_elem:
                                date_elem = vacancy.find('div', class_='date')
                            ...
                            print(f"   📅 Дата: {date}")
                            print(f"   🔧 Навыки: {', '.join(skills) if skills else 'Не указаны'}")
                            print(f"   🔗 Ссылка: {full_link}")
                            print("-" * 80)
                            
                        except Exception as e:
                            print(f"Ошибка при парсинге вакансии {i}: {str(e)[:100]}")
                            continue
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    print(f"🔍 Найдено вакансий в Грозном: {len(vacancies)}\n")
                    
                    if len(vacancies) == 0:
                        print("⚠️ Не найдено вакансий.")
                        return 0
                    ...
                    for i, vacancy in enumerate(vacancies, 1):
                        try:
                            ...
                            print(f"   📅 Дата: {date}")
                            print(f"   🔗 Ссылка: {full_link}")
                            print(f"   📍 Город: Грозный")
                            print("-" * 70)
                            
                        except Exception as e:
                            continue
                    
                    if parsed_count == 0 and len(vacancies) > 0:
                        print("\n⚠️ Найдены элементы, но не удалось извлечь данные.")

                    
                    print(f"\n✅ Найдено вакансий в Грозном: {parsed_count}")
                    return parsed_count
